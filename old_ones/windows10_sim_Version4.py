...
def win1_button(master, **kwargs):
    # Create a button with Windows 1.0 look
    return tk.Button(
        master,
        font=FONT,
        bg=BUTTON_BG,
...
class Notepad(DraggableWindow):
    def __init__(self, master, **kwargs):
        ...
        # OG bottom bar for Notepad
        bottom_bar = tk.Frame(self.frame, bg=WIN_BG, height=18)
        bottom_bar.pack(fill=tk.X, side=tk.BOTTOM)
        win1_button(bottom_bar, text="OK", width=6, command=self.destroy).pack(side=tk.RIGHT, padx=10, pady=2)
...
class PaintApp(DraggableWindow):
    def __init__(self, master, **kwargs):
        ...
        for color in ["#000000", "#0000ff", "#ff0000", "#008000", "#ffff00", "#ffa500", "#ffffff"]:
            cbtn = win1_button(toolbar, bg=color, width=2, text="   ", command=lambda col=color: self.set_color(col))
    ...
    def draw(self, event):
        if self.last_x is not None and self.last_y is not None:
            self.set_width()
            self.canvas.create_line(self.last_x, self.last_y, event.x, event.y, fill=self.current_color, width=self.pen_width, capstyle=tk.PROJECTING, smooth=False)
        self.last_x, self.last_y = event.x, event.y
